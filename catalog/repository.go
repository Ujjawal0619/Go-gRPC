...
type Repository interface {
	...
	ListProductsWtihIDs(ctx context.Context, ids []string) ([]Product, error)
...
func (r *elasticRepository) PutProduct(ctx context.Context, p Product) error {
	_, err := r.client.Index().
		Index("catalog").Type("product").Id(p.ID).BodyJson(productDocument{
		Name:        p.Name,
...
func (r *elasticRepository) ListProducts(ctx context.Context, skip uint64, take uint64) ([]Product, error) {
	res, err := r.client.Search().Index("catalog").Type("product").Query(elastic.NewMatchAllQuery()).
...
func (r *elasticRepository) ListProductsWtihIDs(ctx context.Context, ids []string) ([]Product, error) {
